...
class ExcelFileCreator_XlsxWriter(object):
	...
	def get_data(ivblock, variablenames, avgCol, ivblocklines):
		...
		ExcelFileCreator_XlsxWriter.make_plot(ivblock, variablenames[0], columndata, 1)
		columndata = []
		for k in range(1, len(avgCol) - 1):
			for l in range(avgCol[k] +1, avgCol[k+1]):
				temp[l].pop(0)
				columndata.append(temp[l])
			ExcelFileCreator_XlsxWriter.make_plot(ivblock, variablenames[k], columndata, k + 1) 
			columndata =[]
	...
	def make_plot(ivblock, variablenames, columndata, index):
		...
		for j in range(1, len(columndata)):
			temp = [y * 10**9 for y in columndata[j] if y != ""]
			yvals.append(temp)

		for sublist in range(0,len(yvals)):
			plt.plot(xvals, yvals[sublist], 'o', linestyle = '-', label = variablenames[sublist])
		...
		my_block = ivblock
		title = [my_block.concentration_string(), my_block.electrolyte(), my_block.pH(), my_block.modification(), str(variablenames[0]).split(',')[0], "Graph", str(index)]
		plt_title = ""
		for each in title:
			plt_title += each
			plt_title += " "
		...
		for each in name:
			save_name += each
			save_name += "_"
		save_name += "Graph_" + str(index) + ".png"
	...
	def WriteIVBlockContents(self, sheet, row, column, ivblock):
		...
		ExcelFileCreator_XlsxWriter.get_data(ivblock, variableNames, averageColumns, ivblocklines)
